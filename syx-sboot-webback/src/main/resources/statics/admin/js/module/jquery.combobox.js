...
$.fn.Combobox = function (options, param) {
    ...
    opts = $.extend({}, $.fn.Combobox.defaults, options);
    opts.target = this;
    if (!opts.data) {  //不存在数据
...
$.fn.Combobox.methods = {
    ...
    setValue: function (jq, value) {
        ...
        $(jq).next().find('dd').find('p').each(function () {
            ...
            if (selfVal == value) {
                ...
                $(this).addClass("sui-option-selected").siblings().removeClass("sui-option-selected");
    ...
    setValues: function (jq, values) {
        ...
        $(jq).next().find('dd').find('p').each(function () {
            var selfVal = $(this).data('name');
            $(this).removeClass("sui-option-selected");
            ...
            for (var _i = 0; _i < t_values.length; _i++) {
                if (selfVal == t_values[_i]) {//若value相同
                    ...
                    $(this).addClass("sui-option-selected");
    ...
    init: function (opts) {
        var val = opts.valueField,
            text = opts.textField,
            data = opts.data,
            html = '<dl class="sui-select-panel">',
            dtHtml = ' <dt class="sui-mselect"><a href="javascript:void (0);">',
            ddHtml = '<dd class="sui-option">',
            dText = "",
            isDefaultValue = '',
            choose = opts.target.data("choose");

        if (choose && choose == 'disable') {
            dtHtml = ' <dt class="sui-mselect" style="background-color: #CCCCCC;box-shadow: none;"><a href="javascript:void (0);">';
        }
        ...
        $.each(data, function (i, item) {
            ...
            if (opts.value != null && typeof(clearMh(JSON.stringify(opts.value))) != "undefined") {
                ...
                if (_flog) {
                    ddHtml += '<p href="javascript:void(0)" class="sui-option-selected" data-name="' + _itemVal + '">' + item[text] + '</p>';
                    if (dText) {
                        dText += ",";
                    } else {
                        dText = "";
                ...
                if (item['isDefault']) {
                    ddHtml += '<p  class="sui-option-selected" data-name="' + _itemVal + '">' + item[text] + '</p>';
                    if (dText) {
                        dText += ",";
                    } else {
                        dText = "";
        ...
        opts.dlTarget.find("p").click(function () {
            ...
            if (opts.multiple) {
                ...
                //判断之前是否选中
                if (self.hasClass("sui-option-selected")) {
                    //移除
                    selfVal = removeItem(oldVal, selfVal);
                    selfText = removeItem(oldText, selfText);

                    self.removeClass("sui-option-selected");
                } else {
                    ...
                    self.addClass("sui-option-selected");
                }

            } else {
                self.addClass("sui-option-selected").siblings().removeClass("sui-option-selected");
            }
