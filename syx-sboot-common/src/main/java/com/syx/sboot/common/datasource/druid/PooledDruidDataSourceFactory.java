...
@Component
public class PooledDruidDataSourceFactory extends DataSourceFactory {

    private static final Logger log = LoggerFactory.getLogger(PooledDruidDataSourceFactory.class);
    private static final Map<String, String> codeMap = new ConcurrentHashMap();
    private static final Map<String, PooledDruidDataSource> dsMap = new ConcurrentHashMap();

    public PooledDruidDataSourceFactory() {
    }
    ...
    private TypedDataSource doGetSysDataSource() {
    	PooledDruidDataSource ds = (PooledDruidDataSource)dsMap.get("system");
        if(ds != null) {
            return ds;
        } else {
            synchronized(this) {
                ds = (PooledDruidDataSource)dsMap.get("system");
                if(ds != null) {
                    return ds;
                } else {
...
//                                String jdbcPassword = AppConfiguration.getInstance().getString("jdbc.password");
                                ...
                                if(StringUtils.isBlank(jdbcPassword)) {
                                    ...
                                    ds = new PooledDruidDataSource(jdbcDriver, jdbcUrl, jdbcUsername, jdbcPassword);
...
//                                    String poolPingQuery = AppConfiguration.getInstance().getString("pool.pingQuery", "select 1 ");
...
//                                    int poolPingTime = AppConfiguration.getInstance().getInt("pool.pingTime", 0);
    ...
    private TypedDataSource doGetAppDataSourceById(String dsId) {
    	PooledDruidDataSource ds = (PooledDruidDataSource)dsMap.get(dsId);
        if(ds != null) {
            return ds;
        } else {
            synchronized(this) {
                ds = (PooledDruidDataSource)dsMap.get(dsId);
                if(ds != null) {
                    return ds;
                } else {
    ...
    private TypedDataSource doGetAppDataSourceByCode(String Dscode) {
        ...
        synchronized(this) {
            ...
            PooledDruidDataSource entDataSource;
            if(dsId != null) {
                entDataSource = (PooledDruidDataSource)dsMap.get(dsId);
                if(entDataSource != null) {
            ...
            PooledDruidDataSource ds2 = this.createDataSource(entDataSource1);
    ...
    private PooledDruidDataSource createDataSource(SysDataSource entDataSource) {
            ...
            if(StringUtils.isBlank(jdbcUrl)) {
                ...
                if(StringUtils.isBlank(jdbcUsername)) {
                    ...
                    if(StringUtils.isBlank(jdbcPassword)) {
                        ...
                        PooledDruidDataSource ds = new PooledDruidDataSource(jdbcDriver, jdbcUrl, jdbcUsername, jdbcPassword);
                        ds.setDefaultAutoCommit(true);
    ...
    public void resetDataSourceById(String dataSourceId) {
        if(!StringUtils.isBlank(dataSourceId)) {
            ...
            if(dsMap.containsKey(dsId)) {
                synchronized(this) {
                    if(dsMap.containsKey(dsId)) {
                        PooledDruidDataSource dataSource = (PooledDruidDataSource)dsMap.remove(dsId);
