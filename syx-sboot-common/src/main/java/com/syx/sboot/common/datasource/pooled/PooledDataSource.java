...
import com.syx.sboot.common.datasource.unpooled.UnpooledDataSource;
...
public class PooledDataSource implements TypedDataSource {
    ...
    public boolean isDefaultAutoCommit() {
        return this.dataSource.isDefaultAutoCommit().booleanValue();
    }
        ...
        synchronized(this.state) {
            if(this.state.activeConnections.size() > 0 || this.state.idleConnections.size() > 0) {
                ...
                for(i = this.state.activeConnections.size(); i > 0; --i) {
                    try {
                        e = (PooledConnection)this.state.activeConnections.remove(i - 1);
                        e.invalidate();
                ...
                for(i = this.state.idleConnections.size(); i > 0; --i) {
                    try {
                        e = (PooledConnection)this.state.idleConnections.remove(i - 1);
                        e.invalidate();
                        realConn = e.getRealConnection();
                        if(!realConn.getAutoCommit()) {
                            realConn.rollback();
        ...
        synchronized(this.state) {
            ...
            if(conn.isValid()) {
                ...
                if(this.state.idleConnections.size() < this.poolMaximumIdleConnections && conn.getConnectionTypeCode() == this.expectedConnectionTypeCode) {
                    ...
                    PooledConnection newConn = new PooledConnection(realConn, this);
